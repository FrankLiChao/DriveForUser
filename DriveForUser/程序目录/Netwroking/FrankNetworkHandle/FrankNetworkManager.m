...
@implementation FrankNetworkManager
+ (instancetype)allocWithZone:(struct _NSZone *)zone
{
    static FrankNetworkManager *_manager;
                 ...
                 showHUD:(BOOL)showHUD
{
    ...
    [[FrankNetworkHandler sharedInstance] conURL:url networkType:FrankNetWorkGET params:mutableDict delegate:delegate showHUD:showHUD target:target action:action successBlock:successBlock failureBlock:failureBlock];
}
/**
 *   GET请求通过Block 回调结果
 *
...
+ (void)getRequstWithURL:(NSString*)url
                  params:(NSDictionary*)params
                delegate:(id<FrankNetworkDelegate>)delegate
                 showHUD:(BOOL)showHUD
{
...
{
    NSMutableDictionary *mutableDict = [NSMutableDictionary dictionaryWithDictionary:params];
    [[FrankNetworkHandler sharedInstance] conURL:url networkType:FrankNetWorkPOST params:mutableDict delegate:delegate showHUD:showHUD target:target action:action successBlock:successBlock failureBlock:failureBlock];
}
/**
 *   通过 Block回调结果
 *
                  ...
                  showHUD:(BOOL)showHUD
{
    ...
    FLLog(@"上传图片接口 URL-> %@",url);
    FLLog(@"上传图片的参数-> %@",paramsDict);
...
+ (void)uploadFilesWithURL:(NSString*)url
              ...
              successBlock:(FrankSuccessBlock)successBlock
              failureBlock:(FrankFailureBlock)failureBlock
               uploadFiles:(NSMutableArray *)uploadFiles
                   showHUD:(BOOL)showHUD
{
    if (showHUD) {
        [MBProgressHUD showHUDAddedTo:nil animated:YES];
    ...
    FLLog(@"上传图片的参数-> %@",paramsDict);
    AFHTTPSessionManager *manager = [AFHTTPSessionManager manager];
//    for (MHUploadParam *uploadParam in uploadFiles) {
    for (int i=0;i<uploadFiles.count;i++) {
        FLLog(@"%@",uploadFiles[i]);
        UIImage *image = uploadFiles[i];
