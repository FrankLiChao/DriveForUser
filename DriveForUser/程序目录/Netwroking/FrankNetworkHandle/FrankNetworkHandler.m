...
#import "FrankNetworkItem.h"
#import "AFNetworking.h"
@interface FrankNetworkHandler ()
<FrankNetworkDelegate>
@end;
@implementation FrankNetworkHandler

+ (FrankNetworkHandler *)sharedInstance
{
    static FrankNetworkHandler *handler = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        handler = [[FrankNetworkHandler alloc] init];
    });
    return handler;
}
               ...
               failureBlock:(FrankFailureBlock)failureBlock
{
    ...
    self.netWorkItem = [[FrankNetworkItem alloc]initWithtype:networkType url:url params:params delegate:delegate target:target action:action hashValue:hashValue showHUD:showHUD successBlock:successBlock failureBlock:failureBlock];
    ...
    [mgr setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        ...
        switch (status)
        {
            case AFNetworkReachabilityStatusUnknown: // 未知网络
                FLLog(@"未知网络");
                [FrankNetworkHandler sharedInstance].networkError = NO;
                break;
            case AFNetworkReachabilityStatusNotReachable: // 没有网络(断网)
                FLLog(@"没有网络");
                [FrankNetworkHandler sharedInstance].networkError = YES;
                break;
...
{
    FrankNetworkHandler *handler = [FrankNetworkHandler sharedInstance];
    [handler.items removeAllObjects];
    handler.netWorkItem = nil;
}

- (void)netWorkWillDealloc:(FrankNetworkItem *)itme
{
