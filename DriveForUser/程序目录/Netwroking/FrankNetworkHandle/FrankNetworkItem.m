...
@interface FrankNetworkItem ()

@end

@implementation FrankNetworkItem
                      ...
                      failureBlock:(FrankFailureBlock)failureBlock
{
    if (self = [super init])
    {
        ...
        if (networkType==FrankNetWorkGET)
        {
            ...
            [newParams setObject:OurRequestSignStr forKey:@"appKey"];
            [newParams setObject:[lhColor signStrOur:newParams] forKey:@"sign"];
            [newParams removeObjectForKey:@"appKey"];
            
            [manager POST:url parameters:newParams success:^(NSURLSessionDataTask * _Nonnull task, id responseObject) {
                [MBProgressHUD hideHUDForView:nil animated:YES];
//                DTLog(@"\n\n----请求的返回结果 %@\n",responseObject);
                if (successBlock) {
                    successBlock(responseObject);
