 ...
 *  显示请求失败的信息
 */
+ (void)requestFailAlertShow:(NSString *)msg
{
 ...
 *  根据title和msg向用户显示消息
 */
+ (void)showMessageWithString:(NSString *)title WithMessage:(NSString *)msg;
{
    NSString * string = [NSString stringWithFormat:@"%@",msg];
    UIAlertView * alertView = [[UIAlertView alloc]initWithTitle:title message:string delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
    [alertView show];
}
...
+(NSInteger)getSecondsWithBeginDate:(NSString*)currentDateString  AndEndDate:(NSString*)tomDateString{
    ...
    NSDate * currentDate = [FrankTools NSStringToDate:currentDateString withFormat:@"yyyy-MM-dd HH:mm:ss"];
    NSInteger currSec = [currentDate timeIntervalSince1970];
    
    NSDate *tomDate = [FrankTools NSStringToDate:tomDateString withFormat:@"yyyy-MM-dd HH:mm:ss"];
