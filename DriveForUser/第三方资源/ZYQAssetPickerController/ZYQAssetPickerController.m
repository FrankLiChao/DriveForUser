...
#define kPopoverContentSize CGSizeMake(DeviceMaxWidth, DeviceMaxHeight)
//#define maxPhotoNum 9
#define nowPhotoNum [[NSUserDefaults standardUserDefaults] objectForKey:@"writeMessagePicCountFile"]
...
//    CGSize titleSize        = [self.text sizeWithFont:self.font];
    ...
    CGSize titleSize = [lhColor sizeWithFontWhenIOS7:self.text font:self.font];
...
//         baselineAdjustment:UIBaselineAdjustmentAlignCenters];
    ...
    CGRect rect22 = CGRectMake(rect.size.width - titleSize.width - 2, (height - 12) / 2, kThumbnailLength, CGFLOAT_MAX);
    
    [lhColor sizeWithFontWhenIOS7:self.text font:self.font rect:rect22 forWidth:kThumbnailLength fontSize:12 lineBreakMode:NSLineBreakByTruncatingTail baselineAdjustment:UIBaselineAdjustmentAlignCenters];
...
-(id)initWithFrame:(CGRect)frame{
    selectCount = [nowPhotoNum integerValue];
...
-(void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    if ((_selected=!_selected) && selectCount < [lhColor shareColor].uploadPicNumber) {
        selectCount++;
        self.backgroundColor=selectedColor;
        [_selectView setImage:checkedIcon];
    }
    else if(_selectView.image){
        selectCount--;
...
- (void)setupViews
{
    self.tableView.backgroundColor = [UIColor whiteColor];//改颜色
}
...
{
    NSLog(@"%ld ",_indexPathsForSelectedItems.count+[nowPhotoNum integerValue]);
    if (_indexPathsForSelectedItems.count+[nowPhotoNum integerValue] < [lhColor shareColor].uploadPicNumber) {
...
//    if (_indexPathsForSelectedItems.count > maxPhotoNum) {
        //NSLog(@"够啦额");
...
- (void)setTitleWithSelectedIndexPaths:(NSArray *)indexPaths
{
    ...
    else if(indexPaths.count >= [lhColor shareColor].uploadPicNumber+1){//已经选择六张图片
        //NSLog(@"最多只能选择六张图片  %ld",(long)[nowPhotoNum integerValue]);
    ...
    if (photosSelected && videoSelected)
        ...
        format = (indexPaths.count > 1) ? NSLocalizedString(@"已选择 %ld/%ld 张照片", nil) : NSLocalizedString(@"已选择 %ld/%ld 张照片 ", nil);
    
    else if (videoSelected)
        format = (indexPaths.count > 1) ? NSLocalizedString(@"已选择 %ld/%ld 部视频", nil) : NSLocalizedString(@"已选择 %ld/%ld 部视频 ", nil);
    
    self.title = [NSString stringWithFormat:format, (long)indexPaths.count+[nowPhotoNum integerValue],[lhColor shareColor].uploadPicNumber];
}
...
- (void)setupViews
{
    ...
    self.tableView.backgroundColor = [lhColor colorFromHexRGB:@"DBDBDB"];//改颜色
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == 0) {
        UITapGestureRecognizer * tapGesture = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(tapGestureCamera)];
        
        UIView * cameraView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, DeviceMaxWidth, 88)];
        cameraView.backgroundColor = [UIColor whiteColor];
        [cameraView addGestureRecognizer:tapGesture];
        UIImageView * caImg = [[UIImageView alloc]initWithFrame:CGRectMake(15, 6, 78, 78)];
        caImg.image = imageWithName(@"camera.png");
        [cameraView addSubview:caImg];
        
        UILabel * caLabel = [[UILabel alloc]initWithFrame:CGRectMake(112, 25, 100, 40)];
        caLabel.text = @"拍照";
        [cameraView addSubview:caLabel];
        
        [self.tableView addSubview:cameraView];
        //拍照行
    }
    else{
        [cell bind:[self.groups objectAtIndex:indexPath.row-1]];
        
    }
    UIImageView * foreImg = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, DeviceMaxWidth, 90)];
    foreImg.image = imageWithName(@"cameraCell.png");
    [cell addSubview:foreImg];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.row == 0) {
        ...
        vc.assetsGroup = [self.groups objectAtIndex:indexPath.row-1];
